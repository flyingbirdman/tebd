...
if(!exist("outfile")) outfile=topdf.'corf_mf0_i.pdf'; else outfile=topdf.outfile.'corf_mf0_i.pdf'
if(!exist("yl")) yl=3.2
if(!exist("xl")) xl=6.4
...
set title '(a)'
set ylabel 'Im<D_{k,12}>' offset 0.5, 0.0
if(!exist("prefix")){
	r=3
	plot locfile index r u 1:2 w lp ls 1, "" index r u 1:3 w lp ls 2
}else{
	r=3
	plot for [i=1:numvar] prefix.word(var,i).postfix index r u 1:3 w lp ls i t word(var,i)
}

set title '(b)'
set ylabel ' Im<D_{k,21}>' offset 0.5, 0.0
if(!exist("prefix")){
	r=4
	plot locfile index r u 1:2 w lp ls 1, "" index r u 1:3 w lp ls 2
}else{
...
set title '(c)'
set ylabel 'Im<D_{k,34}>' offset 0.5, 0.0
if(!exist("prefix")){
	r=5
	plot locfile index r u 1:2 w lp ls 1, "" index r u 1:3 w lp ls 2
}else{
...
set title '(d)'
set ylabel 'Im<D_{k,43}>' offset 0.5, 0.0
if(!exist("prefix")){
	r=6
	plot locfile index r u 1:2 w lp ls 1, "" index r u 1:3 w lp ls 2
}else{
